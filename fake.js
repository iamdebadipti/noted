export const allNotes = [
  {
    id: '1',
    title: 'Using redux + redux-persist you can define what is persisted',
    description:
      'Using redux + redux-persist you can define what is persisted and what is not. When not persisted, data exists while the app is running. When persisted, the data persists between app executions (close, open, restart phone, etc).',
    tags: [
      { id: 1, name: 'Ideas' },
      { id: 2, name: 'JavaScript' },
      { id: 3, name: 'React Native' }
    ]
  },
  {
    id: '2',
    title: 'All data you fetched and stored are available',
    description:
      'AsyncStorage has a default limit of 6MB on Android. It is possible to configure a larger limit (on Java code) or use redux-persist-filesystem-storage as storage engine for Android.',
    tags: [
      { id: 1, name: 'Ideas' },
      { id: 2, name: 'JavaScript' },
      { id: 3, name: 'React Native' }
    ]
  },
  {
    id: '3',
    title: 'It is possible to configure a larger limit',
    description: 'React promotes a reactive way of creating apps and Redux fits very well on it.',
    tags: [
      { id: 1, name: 'Ideas' },
      { id: 2, name: 'JavaScript' },
      { id: 3, name: 'React Native' }
    ]
  },
  {
    id: '4',
    title: 'Using redux + redux-persist you can define what is persisted',
    description:
      'Using redux + redux-persist you can define what is persisted and what is not. When not persisted, data exists while the app is running. When persisted, the data persists between app executions (close, open, restart phone, etc).',
    tags: [
      { id: 1, name: 'Ideas' },
      { id: 2, name: 'JavaScript' },
      { id: 3, name: 'React Native' }
    ]
  },
  {
    id: '5',
    title: 'All data you fetched and stored are available',
    description:
      'AsyncStorage has a default limit of 6MB on Android. It is possible to configure a larger limit (on Java code) or use redux-persist-filesystem-storage as storage engine for Android.',
    tags: [
      { id: 1, name: 'Ideas' },
      { id: 2, name: 'JavaScript' },
      { id: 3, name: 'React Native' }
    ]
  },
  {
    id: '6',
    title: 'It is possible to configure a larger limit',
    description: 'React promotes a reactive way of creating apps and Redux fits very well on it.',
    tags: [
      { id: 1, name: 'Ideas' },
      { id: 2, name: 'JavaScript' },
      { id: 3, name: 'React Native' }
    ]
  }
];
